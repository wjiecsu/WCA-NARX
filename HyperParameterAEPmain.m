...
for i=1:length(corr_thetaSet)
    ...
    for j=1:length(Ratio_thetaSet)
        ...
        [yPredTest(:,(i-1)*length(Ratio_thetaSet)+j),runtime(1,(i-1)*length(Ratio_thetaSet)+j)]= WCA_NARX(XInput1,XInput2,Ytrain,XtestInput1,XtestInput2,nMFs,alpha,beta1,beta2,lambda,corr_theta,Ratio_theta,MaxEpoch);
    end
end
%----------------------Performance-----------------------------------------------------------
...
csvwrite('./Result/HyPerAEPR2Test.csv',R2Test);
csvwrite('./Result/HyPerAEPRuntime.csv',runtime);
